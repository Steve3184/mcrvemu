...
def generate_vt_init(dist_dir):
    vt_list = [["-" * 80 + "\\n"]]
    for _ in range(25):
        vt_list.extend([[""], ["\\n"]])
    vt_list.append(["-" * 80])
    json_vt = json.dumps(vt_list)
    
    vt_dir = f"{dist_dir}/data/rv/function/core/vt"
    os.makedirs(vt_dir, exist_ok=True)
    with open(f"{vt_dir}/init.mcfunction", 'w', encoding='utf-8') as f:
        f.write("# Generated by build.py\n")
        f.write("scoreboard players set #vt_lptr rv_data 1\n")
        f.write(f"data modify storage rv:data uart.vt set value {json_vt}\n")
...
def main():
    ...
    generate_vt_init(dist_dir)

    with open(f"{dist_dir}/data/rv/function/core/step_interrupt.mcfunction", 'w') as f:
        f.write("scoreboard players operation #old_mtime rv_data = mtime rv_data\n")
        f.write(f"scoreboard players add mtime rv_data {INTERRUPT_INTERVAL}\n")
        f.write("execute if score #old_mtime rv_data matches ..-1 if score mtime rv_data matches 0.. run scoreboard players add mtimeh rv_data 1\n")
        ...
        f.write("scoreboard players set #mtimecmp_nonzero rv_data 0\n")
        f.write("execute unless score mtimecmp rv_data matches 0 run scoreboard players set #mtimecmp_nonzero rv_data 1\n")
        f.write("execute unless score mtimecmph rv_data matches 0 run scoreboard players set #mtimecmp_nonzero rv_data 1\n")
        ...
        f.write("execute if score #result rv_data matches 1 if score #mtimecmp_nonzero rv_data matches 1 run scoreboard players add mip rv_data 128\n")
        
        f.write("scoreboard players operation #val1 rv_data = mip rv_data\n")
        f.write("scoreboard players operation #val2 rv_data = mie rv_data\n")
        f.write("function rv:alu/and\n")
        f.write("scoreboard players operation #pending rv_data = #val rv_data\n")
        
        f.write("scoreboard players set #iswfi rv_data 0\n")
        f.write("execute if score #trap rv_data matches -3 unless score #pending rv_data matches 0 run scoreboard players set #iswfi rv_data 1\n")
        f.write("execute if score #iswfi rv_data matches 1 run scoreboard players set #trap rv_data -1\n")
        ...
        f.write("execute if score #iswfi rv_data matches 1 if score #mstatus_mie rv_data matches 1 unless score #pending rv_data matches 0 run function rv:core/dispatch_interrupt\n")
    ...
    with open(f"{dist_dir}/data/rv/function/core/loop.mcfunction", 'w') as f:
        ...
        for _ in range(groups):
            for _ in range(INTERRUPT_INTERVAL):
                f.write("execute if score #trap rv_data matches -1 run function rv:core/step\n")
            f.write("function rv:core/step_interrupt\n")
        f.write("function rv:core/vt/draw\n")
